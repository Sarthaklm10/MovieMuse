...
export default function useMovies(query, manualSearchTrigger = 0) {
  ...
  const fetchMovies = useCallback(async (searchQuery, controller) => {
    // Check cache
    if (searchCache.has(searchQuery)) return searchCache.get(searchQuery);
  ...
  }, [query, manualSearchTrigger, fetchMovies]);
