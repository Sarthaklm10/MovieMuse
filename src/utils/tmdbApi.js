...
export async function getMovieDetails(movieId) {
  ...
  const id = movieId.startsWith("tmdb-")
...
export function convertTMDBMovie(tmdbMovie) {
  ...
  return {
    ...
    backdrop_path: tmdbMovie.backdrop_path
      ? `${TMDB_IMAGE_URL_ORIGINAL}${tmdbMovie.backdrop_path}`
      : null,
    Type: "movie",
...
export function formatMovieDetails(tmdbDetails) {
  ...
  const audienceScore = tmdbDetails.vote_average
    ? Math.round(tmdbDetails.vote_average * 10)
    : "N/A";

  // Extract IMDb ID if available in external IDs
  const imdbIDValue =
    tmdbDetails.external_ids?.imdb_id || `tmdb-${tmdbDetails.id}`;

  // Extract production company names
  const productionCompanyNames =
    tmdbDetails.production_companies?.map((company) => company.name) || [];
  ...
  return {
    ...
    Year: tmdbDetails.release_date
      ? tmdbDetails.release_date.split("-")[0]
      : "N/A",
    Rated: rated,
    Released: released,
    ...
    Language: tmdbDetails.original_language
      ...
      : "N/A",
    Country:
      tmdbDetails.production_countries?.map((c) => c.name).join(", ") || "N/A",
    Awards: "N/A", // TMDB doesn't provide awards directly
    Poster: tmdbDetails.poster_path
      ? `${TMDB_IMAGE_URL}${tmdbDetails.poster_path}`
      : "N/A",
    Ratings: [
    ...
    imdbRating: tmdbDetails.vote_average
      ...
      : "N/A",
    imdbVotes: tmdbDetails.vote_count
      ? tmdbDetails.vote_count.toString()
      : "N/A",
    Type: "movie",
    DVD: tmdbDetails.release_dates?.results
      ?.find((r) => r.iso_3166_1 === "US")
      ?.release_dates?.find((d) => d.type === 4 || d.type === 5)?.release_date
      ? new Date(
          tmdbDetails.release_dates.results
            .find((r) => r.iso_3166_1 === "US")
            .release_dates.find(
              (d) => d.type === 4 || d.type === 5
            ).release_date
    ...
    BoxOffice: tmdbDetails.revenue
      ? `$${tmdbDetails.revenue.toLocaleString()}`
      : "N/A",
    Production:
...
export async function findTMDBId(imdbId) {
  ...
  const data = await fetchTMDB(`/find/${imdbId}`, {
